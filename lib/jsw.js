...
const { sendEventsToAll, updateSdkKey } = require("../routes/index");
...
class JetstreamWrapper {
  ...
  async _subscribeToFullRuleSet() {
    ...
    const handler = (m) => {
      ...
      sendEventsToAll(this.sc.decode(m.data));
    }
    this._subscribeToRequests({ streamName: 'DATA', subsetName: 'FullRuleSet', handler });
  ...
  async _subscribeToSdkKey() {
    ...
    const handler = (m) => {
      console.log(`SDK key received: ${this.sc.decode(m.data)}`)
      updateSdkKey(this.sc.decode(m.data));
    }
